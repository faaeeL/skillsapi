...
public class SkillConfigParser {
    ...
    private static SkillEffect parseShapeEffect(Map<?, ?> raw, Plugin plugin, StatusManager statusManager, SummonManager summonManager, ThreatManager threatManager) {
        ...
        double hitRadius = toDouble(raw.get("hit_radius"), 0);
        ShapeEffect.HitArea hitArea = switch (raw.get("hit_area") == null ? "points" : raw.get("hit_area").toString().toLowerCase(Locale.ROOT)) {
            case "disk" -> ShapeEffect.HitArea.DISK;
            case "sphere" -> ShapeEffect.HitArea.SPHERE;
            default -> ShapeEffect.HitArea.POINTS;
        };
        double hitHeight = toDouble(raw.get("hit_height"), 2.0);
        boolean hitOnce = toBool(raw.get("hit_once"), true);
        int hitIntervalTicks = toInt(raw.get("hit_interval_ticks"), intervalTicks);
        boolean includeSelf = toBool(raw.get("include_self"), false);
        ...
        boolean disarmAfterHit = toBool(raw.get("disarm_after_hit"), false);

        List<SkillEffect> onHit = parseEffectList(asMapList(raw.get("on_hit")), plugin, statusManager, summonManager, threatManager);
    ...
    private static SkillEffect parseProjectile(Map<?, ?> raw, Plugin plugin, StatusManager statusManager, SummonManager summonManager, ThreatManager threatManager) {
        ...
        List<SkillEffect> onHit = parseEffectList(asMapList(raw.get("effects")), plugin, statusManager, summonManager, threatManager);
        ...
        return new ProjectileEffect(
                ...
                toDouble(raw.get("hit_radius"), 1.0),
                toInt(raw.get("pierce"), 1),
    ...
    private static SkillEffect parseSummon(Map<?, ?> raw, Plugin plugin, StatusManager statusManager, SummonManager summonManager, ThreatManager threatManager) {
        ...
        SummonManager.CapBehavior onCapExceeded = "dismiss_oldest".equalsIgnoreCase(String.valueOf(raw.get("on_cap_exceeded")))
        ...
        return new SummonEffect(
                ...
                toInt(raw.get("max_active"), 0),
                onCapExceeded,
                toDouble(raw.get("aggro_radius"), 12),
                toDouble(raw.get("follow_radius"), 10),
                toDouble(raw.get("move_speed"), 1.0),
                toInt(raw.get("ai_interval_ticks"), 10),
                onSummon
        );
    }
